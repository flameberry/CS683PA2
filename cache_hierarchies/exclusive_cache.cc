...
void CACHE::handle_fill() {
	...
	if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
		...
		if (cache_type == IS_L1I || cache_type == IS_L1D || cache_type == IS_L2C || cache_type == IS_LLC) {
			...
			if (cache_type == IS_L2C && do_bypass) {
			...
			if (do_bypass && (cache_type == IS_L1D || cache_type == IS_L2C)) {
		...
		// todo: add L1I
		if (cache_type == IS_L1D || cache_type == IS_L2C || block[set][way].dirty) {
		...
		if (do_fill) {
			if (!do_bypass) {
				// (Aditya): Debug
				// cout << "[" << this->NAME << "]: Evicting address: " << std::hex << block[set][way].address << " from set: " << std::dec << set << ", way: " << way << "; Filling in address: " << std::hex << MSHR.entry[MSHR.next_fill_index].address << std::dec << endl;
				// (Aditya): -----
	...
	if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
		...
		if (way >= 0) { // writeback hit (or RFO hit for L1D)
			...
			if (cache_type == IS_L1D) { // RFO miss
				...
				if (block[set][way].dirty) {
					...
					if (lower_level) {
						if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
							...
							// (Aditya): Will this cause no writeback hits?
							// invalidate_entry(writeback_packet.address);
							// (Aditya): ----------------------------------
						}
					}
#ifdef SANITY_CHECK
				...
				if (do_fill) {
					// (Aditya): Debug
					// cout << "[Writeback: " << this->NAME << "]: Evicting address: " << std::hex << block[set][way].address << " from set: " << std::dec << set << ", way: " << way << "; Filling in address: " << std::hex << WQ.entry[index].address << std::dec << endl;
					// (Aditya): -----
	...
	for (uint32_t i = 0; i < MAX_READ; i++) {
		...
		if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
			...
			if ((cache_type == IS_L1D || cache_type == IS_L1I) && (PQ.entry[PQ.head].full_physical_address >> LOG2_PAGE_SIZE) == (UINT64_MAX >> LOG2_PAGE_SIZE)) {
				...
				DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch on dropping prefetch request" << "instruction : " << PQ.entry[index].instruction << endl; });
			...
			if (way >= 0) { // prefetch hit
				...
				if (PQ.entry[index].pf_origin_level < fill_level) {
					if (cache_type == IS_L1D) {
						...
						DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch on prefetch hit" << "instruction : " << PQ.entry[index].instruction << endl; });
					} else if (cache_type == IS_DTLB) {
#ifdef SANITY_CHECK
						...
						DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch on prefetch hit" << "instruction " << PQ.entry[index].instruction << "  address: " << PQ.entry[index].address << endl; });
					}
				}
							...
							if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address))
								...
								if (PQ.entry[index].pf_origin_level < fill_level) {
									if (cache_type == IS_L1D)
										...
										DP(if (warmup_complete[PQ.entry[index].cpu]) { cout << "[" << NAME << "_PQ] " << __func__ << " prefetch_id: " << PQ.entry[index].prefetch_id << "from handle prefetch" << "instruction: " << PQ.entry[index].instruction << endl; });
									} else if (cache_type == IS_DTLB) {
#ifdef SANITY_CHECK
										if (PQ.entry[index].instruction) {
											cout << "DTLB prefetch packet should not prefetch address translation of instruction" << endl;
